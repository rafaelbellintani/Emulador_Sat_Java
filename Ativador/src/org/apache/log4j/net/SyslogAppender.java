...
public class SyslogAppender extends AppenderSkeleton
{
    ...
    public synchronized void close() {
        ...
        if (this.sqw != null) {
            try {
                if (this.layoutHeaderChecked && super.layout != null && super.layout.getFooter() != null) {
                    this.sendLayoutMessage(super.layout.getFooter());
                }
                this.sqw.close();
                this.sqw = null;
            }
    ...
    public void append(final LoggingEvent event) {
        ...
        if (!this.layoutHeaderChecked) {
            if (super.layout != null && super.layout.getHeader() != null) {
                this.sendLayoutMessage(super.layout.getHeader());
            }
            this.layoutHeaderChecked = true;
        }
        final String hdr = this.getPacketHeader(event.timeStamp);
        String packet = super.layout.format(event);
    ...
    private String getPacketHeader(final long timeStamp) {
        if (this.header) {
            final StringBuffer buf = new StringBuffer(this.dateFormat.format(new Date(timeStamp)));
            if (buf.charAt(4) == '0') {
                buf.setCharAt(4, ' ');
            }
    ...
    private void sendLayoutMessage(final String msg) {
        if (this.sqw != null) {
            String packet = msg;
            final String hdr = this.getPacketHeader(new Date().getTime());
            if (this.facilityPrinting || hdr.length() > 0) {
                final StringBuffer buf = new StringBuffer(hdr);
                if (this.facilityPrinting) {
                    buf.append(this.facilityStr);
