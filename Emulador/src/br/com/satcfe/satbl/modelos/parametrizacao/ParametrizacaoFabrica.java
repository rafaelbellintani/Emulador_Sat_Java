...
public class ParametrizacaoFabrica
{
    private Node raiz;
    
    public ParametrizacaoFabrica() {
        this.raiz = null;
    }
    
    public ParametrizacaoFabrica(final String xml) {
        this.raiz = null;
    ...
    public boolean parse() {
    	return this.raiz != null;/* && this.parse(this.raiz);*/ //Entra em LOOP se ficar ativado
    }
    ...
    public boolean parse(final Node raiz) {
        try {
            ...
            for (int i = 0; i < filhos.getLength(); ++i) {
                ...
                if (filhoAtual.getNodeName().equalsIgnoreCase("conexao")) {
                ...
                else if (filhoAtual.getNodeName().equalsIgnoreCase("mensagem")) {
                    Parametrizacoes.mensagemAmbiente = filhoAtual.getTextContent();
                }
            }
    ...
    //DETERMINA ONDE O ARQUIVO DE PARAMETRIZACAO FICARÁ
    public static boolean existe() {
    	if(System.getProperty("os.name").equals("Windows")) {
    		return ControleArquivos.existeArquivo(Configuracoes.SistemaArquivos.CAMINHO_ARQUIVO_PARAMETRIZACAO_FABRICA_WINDOWS);
    	}else{
    		return ControleArquivos.existeArquivo(Configuracoes.SistemaArquivos.CAMINHO_ARQUIVO_PARAMETRIZACAO_FABRICA_LINUX);
    	}
    }
}
